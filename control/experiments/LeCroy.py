#!/usr/bin/python
class WS3024:
    def __init__(self, idVendor=0x05ff, idProduct=0x1023, address="192.168.0.16", interface="VXI11"):
        if interface.upper()=="VXI11":
            import vxi11
            self.address = address
            self.connection = vxi11.Instrument(self.address)
        elif interface.upper()=="USBTMC":
...
def readfile(filename):
    ...
    with open(filename, 'rb') as h:
        raw = mmap.mmap(h.fileno(),0,access=mmap.ACCESS_READ)

    firstComma = raw.find(b',')
    cmd = raw[:firstComma]
    dat = raw[firstComma+1:]

    n = int(dat[1:2]) # number of digits in length of data
    N = int(dat[2:2+n]) # number describing length of data
    ...
    headerBytes  = wav[:346]
    headerString = ''.join(map(chr, headerBytes))
    waveformData = wav[346:]
    from struct import unpack

    # Code to parse WAVEDESC generated by parsing template, returned from scope query "TEMPLATE?"
    # Note that this is not well tested and will not handle unusual settings
    WAVEDESC = dict()
    WAVEDESC['DESCRIPTOR_NAME'] = headerString[0:16].strip('\x00')
    WAVEDESC['TEMPLATE_NAME'] = headerString[16:32].strip('\x00')
    WAVEDESC['COMM_TYPE'] = {0: 'byte',1: 'word'}[unpack("<H", headerBytes[32:34])[0]]
    WAVEDESC['COMM_ORDER'] = {0: 'HIFIRST',1: 'LOFIRST'}[unpack("<H", headerBytes[34:36])[0]]
    WAVEDESC['WAVE_DESCRIPTOR'] = unpack('<l', headerBytes[36:40])[0]
    WAVEDESC['USER_TEXT'] = unpack('<l', headerBytes[40:44])[0]
    WAVEDESC['RES_DESC1'] = unpack('<l', headerBytes[44:48])[0]
    WAVEDESC['TRIGTIME_ARRAY'] = unpack('<l', headerBytes[48:52])[0]
    WAVEDESC['RIS_TIME_ARRAY'] = unpack('<l', headerBytes[52:56])[0]
    WAVEDESC['RES_ARRAY1'] = unpack('<l', headerBytes[56:60])[0]
    WAVEDESC['WAVE_ARRAY_1'] = unpack('<l', headerBytes[60:64])[0]
    WAVEDESC['WAVE_ARRAY_2'] = unpack('<l', headerBytes[64:68])[0]
    WAVEDESC['RES_ARRAY2'] = unpack('<l', headerBytes[68:72])[0]
    WAVEDESC['RES_ARRAY3'] = unpack('<l', headerBytes[72:76])[0]
    WAVEDESC['INSTRUMENT_NAME'] = headerString[76:92].strip('\x00')
    WAVEDESC['INSTRUMENT_NUMBER'] = unpack('<l', headerBytes[92:96])[0]
    WAVEDESC['TRACE_LABEL'] = headerString[96:112].strip('\x00')
    WAVEDESC['RESERVED1'] = unpack('<h', headerBytes[112:114])[0]
    WAVEDESC['RESERVED2'] = unpack('<h', headerBytes[114:116])[0]
    WAVEDESC['WAVE_ARRAY_COUNT'] = unpack('<l', headerBytes[116:120])[0]
    WAVEDESC['PNTS_PER_SCREEN'] = unpack('<l', headerBytes[120:124])[0]
    WAVEDESC['FIRST_VALID_PNT'] = unpack('<l', headerBytes[124:128])[0]
    WAVEDESC['LAST_VALID_PNT'] = unpack('<l', headerBytes[128:132])[0]
    WAVEDESC['FIRST_POINT'] = unpack('<l', headerBytes[132:136])[0]
    WAVEDESC['SPARSING_FACTOR'] = unpack('<l', headerBytes[136:140])[0]
    WAVEDESC['SEGMENT_INDEX'] = unpack('<l', headerBytes[140:144])[0]
    WAVEDESC['SUBARRAY_COUNT'] = unpack('<l', headerBytes[144:148])[0]
    WAVEDESC['SWEEPS_PER_ACQ'] = unpack('<l', headerBytes[148:152])[0]
    WAVEDESC['POINTS_PER_PAIR'] = unpack('<h', headerBytes[152:154])[0]
    WAVEDESC['PAIR_OFFSET'] = unpack('<h', headerBytes[154:156])[0]
    WAVEDESC['VERTICAL_GAIN'] = unpack('<f', headerBytes[156:160])[0]
    WAVEDESC['VERTICAL_OFFSET'] = unpack('<f', headerBytes[160:164])[0]
    WAVEDESC['MAX_VALUE'] = unpack('<f', headerBytes[164:168])[0]
    WAVEDESC['MIN_VALUE'] = unpack('<f', headerBytes[168:172])[0]
    WAVEDESC['NOMINAL_BITS'] = unpack('<h', headerBytes[172:174])[0]
    WAVEDESC['NOM_SUBARRAY_COUNT'] = unpack('<h', headerBytes[174:176])[0]
    WAVEDESC['HORIZ_INTERVAL'] = unpack('<f', headerBytes[176:180])[0]
    WAVEDESC['HORIZ_OFFSET'] = unpack('<d', headerBytes[180:188])[0]
    WAVEDESC['PIXEL_OFFSET'] = unpack('<d', headerBytes[188:196])[0]
    WAVEDESC['VERTUNIT'] = headerString[196:244].strip('\x00')
    WAVEDESC['HORUNIT'] = headerString[244:292].strip('\x00')
    WAVEDESC['HORIZ_UNCERTAINTY'] = unpack('<f', headerBytes[292:296])[0]
    WAVEDESC['TRIGGER_TIME'] = headerString[296:312] # Format time_stamp not implemented
    WAVEDESC['ACQ_DURATION'] = unpack('<f', headerBytes[312:316])[0]
    WAVEDESC['RECORD_TYPE'] = {0: 'single_sweep',1: 'interleaved',2: 'histogram',3: 'graph',4: 'filter_coefficient',5: 'complex',6: 'extrema',7: 'sequence_obsolete',8: 'centered_RIS',9: 'peak_detect'}[unpack("<H", headerBytes[316:318])[0]]
    WAVEDESC['PROCESSING_DONE'] = {0: 'no_processing',1: 'fir_filter',2: 'interpolated',3: 'sparsed',4: 'autoscaled',5: 'no_result',6: 'rolling',7: 'cumulative'}[unpack("<H", headerBytes[318:320])[0]]
    WAVEDESC['RESERVED5'] = unpack('<h', headerBytes[320:322])[0]
    WAVEDESC['RIS_SWEEPS'] = unpack('<h', headerBytes[322:324])[0]
    WAVEDESC['TIMEBASE'] = {0: '1_ps/div',1: '2_ps/div',2: '5_ps/div',3: '10_ps/div',4: '20_ps/div',5: '50_ps/div',6: '100_ps/div',7: '200_ps/div',8: '500_ps/div',9: '1_ns/div',10: '2_ns/div',11: '5_ns/div',12: '10_ns/div',13: '20_ns/div',14: '50_ns/div',15: '100_ns/div',16: '200_ns/div',17: '500_ns/div',18: '1_us/div',19: '2_us/div',20: '5_us/div',21: '10_us/div',22: '20_us/div',23: '50_us/div',24: '100_us/div',25: '200_us/div',26: '500_us/div',27: '1_ms/div',28: '2_ms/div',29: '5_ms/div',30: '10_ms/div',31: '20_ms/div',32: '50_ms/div',33: '100_ms/div',34: '200_ms/div',35: '500_ms/div',36: '1_s/div',37: '2_s/div',38: '5_s/div',39: '10_s/div',40: '20_s/div',41: '50_s/div',42: '100_s/div',43: '200_s/div',44: '500_s/div',45: '1_ks/div',46: '2_ks/div',47: '5_ks/div',100: 'EXTERNAL'}[unpack("<H", headerBytes[324:326])[0]]
    WAVEDESC['VERT_COUPLING'] = {0: 'DC_50_Ohms',1: 'ground',2: 'DC_1MOhm',3: 'ground',4: 'AC_1MOhm'}[unpack("<H", headerBytes[326:328])[0]]
    WAVEDESC['PROBE_ATT'] = unpack('<f', headerBytes[328:332])[0]
    WAVEDESC['FIXED_VERT_GAIN'] = {0: '1_uV/div',1: '2_uV/div',2: '5_uV/div',3: '10_uV/div',4: '20_uV/div',5: '50_uV/div',6: '100_uV/div',7: '200_uV/div',8: '500_uV/div',9: '1_mV/div',10: '2_mV/div',11: '5_mV/div',12: '10_mV/div',13: '20_mV/div',14: '50_mV/div',15: '100_mV/div',16: '200_mV/div',17: '500_mV/div',18: '1_V/div',19: '2_V/div',20: '5_V/div',21: '10_V/div',22: '20_V/div',23: '50_V/div',24: '100_V/div',25: '200_V/div',26: '500_V/div',27: '1_kV/div'}[unpack("<H", headerBytes[332:334])[0]]
    WAVEDESC['BANDWIDTH_LIMIT'] = {0: 'off',1: 'on'}[unpack("<H", headerBytes[334:336])[0]]
    WAVEDESC['VERTICAL_VERNIER'] = unpack('<f', headerBytes[336:340])[0]
    WAVEDESC['ACQ_VERT_OFFSET'] = unpack('<f', headerBytes[340:344])[0]
    WAVEDESC['WAVE_SOURCE'] = {0: 'CHANNEL_1',1: 'CHANNEL_2',2: 'CHANNEL_3',3: 'CHANNEL_4',9: 'UNKNOWN'}[unpack("<H", headerBytes[344:346])[0]]

    if len(waveformData) != WAVEDESC['WAVE_ARRAY_1']:
            raise Exception('Binary data not the expected length')

    from numpy import fromstring, int16, arange
    integers = fromstring(waveformData, dtype=int16)
    y = integers * WAVEDESC['VERTICAL_GAIN'] - WAVEDESC['VERTICAL_OFFSET'] 
    x = arange(len(integers)) * WAVEDESC['HORIZ_INTERVAL'] + WAVEDESC['HORIZ_OFFSET']
